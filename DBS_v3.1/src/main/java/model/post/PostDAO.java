...
public class PostDAO {
	...
	String sql_insert = "insert into post(member_id, post_date, post, likes, member_name) values (?,NOW(),?,?,?)"; // 글 작성																												// 진행
	...
	String sql_selectAll = "select * from post order by post_id desc"; // 전체 글 가져오기 
	String sql_insertC = "insert into comment(post_id, member_id, comment, comment_date, member_name) values(?,?,?,now(),?)"; // 댓글 작성
	String sql_deleteC = "delete from comment where comment_id=?"; // 댓글 작성자만이 삭제 가능 
