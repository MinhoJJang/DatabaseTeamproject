...
<c:forEach var="v" items="${datas}">
	<div class="a">
		...
		${pvo.member_id}님의 글: ${pvo.post} [좋아요: ${pvo.likes} | 댓글: ${pvo.comment_count} | ${pvo.post_date}] <mytag:removeB mid="${pvo.member_id}" pid="${pvo.post_id}" />
		
		
		<div class="b">
		
			<c:forEach var="vv" items="${v.cdatas}">
				${vv.member_id}님의 댓글: ${vv.comment} <mytag:removeR mid="${vv.member_id}" cid="${vv.comment_id}" /> <br>
			</c:forEach>
